...
# 参数解析
parser = argparse.ArgumentParser(description='English-Chinese Translation Training')
...
class TranslationDataset:
        ...
        with open(self.data_path, 'r', encoding='utf-8') as f:
            ...
            for i, line in enumerate(lines):
                ...
                if len(parts) == 2:
                    ...
                    self.data.append((english.strip(), chinese.strip()))
    ...
    def _load_from_mindrecord(self):
        ...
        for record in reader.get_next():
            en_text, zh_text = record['src'], record['tgt']
    ...
    def save_vocabularies(self, vocab_dir):
        ...
        vocab_stats = {
            ...
            "source_vocab_size": self.en_vocab_size,
            "target_vocab_size": self.zh_vocab_size,
    ...
    def save_to_mindrecord(self):
        ...
        for en, zh in self.data:
            ...
            data = {
                "src": np.array(en_tokens, dtype=np.int32),
                "tgt": np.array(zh_tokens, dtype=np.int32)
            }
    ...
    def pad_or_truncate(self, tokens):
        if len(tokens) > self.max_length:
            ...
            return tokens + [self.en_token_to_id[PAD_TOKEN]] * (self.max_length - len(tokens))

    def __len__(self):
        return len(self.data)

    def __getitem__(self, idx):
        en_text, zh_text = self.data[idx]
        ...
        return np.array(en_tokens, dtype=np.int32), np.array(zh_tokens, dtype=np.int32)
...
def create_dataset(data_path, batch_size=32, max_length=64):
    ...
    return mindspore_dataset, dataset.en_vocab_size, dataset.zh_vocab_size
...
def train():
    logger.info("Starting English to Chinese translation model training")
    ...
    logger.info(f"Source vocabulary size (English): {src_vocab_size}")
    logger.info(f"Target vocabulary size (Chinese): {tgt_vocab_size}")
    ...
    for epoch in range(args.epochs):
        ...
        for data in dataset.create_dict_iterator():
            ...
            accuracy = np.mean(np.equal(predicted_ids, labels.asnumpy()))  # 计算准确率
        ...
        if (epoch + 1) % args.save_interval == 0:
            model_path = os.path.join(args.output_dir, f"en_zh_translation_model_epoch_{epoch + 1}.ckpt")
    ...
    final_model_path = os.path.join(args.output_dir, "en_zh_translation_model_final.ckpt")
    ...
    with open(config_path, 'w') as f:
        ...
        f.write(f"Source vocabulary size (English): {src_vocab_size}\n")
        f.write(f"Target vocabulary size (Chinese): {tgt_vocab_size}\n")
